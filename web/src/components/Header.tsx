                ...
                <div>
                  ...
                  <ul className="space-y-1 text-xs text-gray-600">
                    {['Yonex (Nhật Bản)', 'Lining (Trung Quốc)', 'Victor (Đài Loan)', 'Kumpoo (Quốc dân)', 'Mizuno (Nhật Bản)'].map((b, i) => (
                      <li key={i} onClick={() => setView('catalog', { brand: b.split(' ')[0] })} className="hover:text-brand-primary hover:pl-1 transition-all py-1">Vợt {b}</li>
                    ))}
                  </ul>
                </div>
                <div>
                  ...
                  <ul className="space-y-1 text-xs text-gray-600">
                    <li onClick={() => setView('catalog', { characteristics: 'Tấn Công' })} className="hover:text-brand-primary hover:pl-1 transition-all py-1 flex items-center gap-1.5">
                      <span className="w-1.5 h-1.5 rounded-full bg-red-500"></span> Thiên công (Heavy Head)
                    </li>
                    <li onClick={() => setView('catalog', { characteristics: 'Phòng Thủ' })} className="hover:text-brand-primary hover:pl-1 transition-all py-1 flex items-center gap-1.5">
                      <span className="w-1.5 h-1.5 rounded-full bg-blue-500"></span> Phản tạt / Tốc độ (Speed)
                    </li>
                    <li onClick={() => setView('catalog', { characteristics: 'Toàn Diện' })} className="hover:text-brand-primary hover:pl-1 transition-all py-1 flex items-center gap-1.5">
                      <span className="w-1.5 h-1.5 rounded-full bg-green-500"></span> Công thủ toàn diện (Even)
                    </li>
                    <li onClick={() => setView('catalog', { characteristics: 'Người Mới' })} className="hover:text-brand-primary hover:pl-1 transition-all py-1 flex items-center gap-1.5">
            ...
            <div className="relative group cursor-pointer py-1 text-gray-800 hover:text-brand-primary transition flex items-center gap-1 shrink-0">
              ...
              <div className="absolute top-full left-0 mt-3 w-[220px] bg-white border border-gray-100 rounded-lg shadow-sm p-3 hidden group-hover:block z-50">
                <ul className="space-y-1.5 text-xs text-gray-600">
                  <li onClick={() => setView('catalog', { category: 'Giày', isWide: true })} className="hover:text-brand-primary py-1 flex items-center justify-between">
                  ...
                  <li onClick={() => setView('catalog', { category: 'Giày', brand: 'Yonex' })} className="hover:text-brand-primary py-1">Giày Yonex Power Cushion</li>
                  <li onClick={() => setView('catalog', { category: 'Giày', brand: 'Lining' })} className="hover:text-brand-primary py-1">Giày Lining Chuyên Dụng</li>
                </ul>
              </div>
            </div>

            {/* Other Categories */}
            <button onClick={() => setView('catalog', { category: 'Túi xách' })} className="text-gray-800 hover:text-brand-primary transition shrink-0 py-1">Bao Vợt & Balo</button>
            <button onClick={() => setView('catalog', { category: 'Cước' })} className="text-gray-800 hover:text-brand-primary transition shrink-0 py-1">Cước Đan Vợt</button>
            <button onClick={() => setView('catalog', { category: 'Quả cầu' })} className="text-gray-800 hover:text-brand-primary transition shrink-0 py-1">Quả Cầu Lông</button>
            
            <button onClick={() => setView('blog-list')} className="text-gray-800 hover:text-brand-primary transition shrink-0 py-1">Đánh Giá Sân Bãi</button>
          </div>
