...
1. Truy cập **PMI Frontend** (mặc định: `http://localhost:3000`).
...
1. Truy cập **WMS Desktop** (mặc định: `http://localhost:3002`).
...
5. Thủ kho dùng điện thoại vào **WMS Mobile** (`http://localhost:3002/m/receive`), quét barcode trên hộp sản phẩm để ghi nhận số lượng thực tế nhận.
...
1. Truy cập **OMS Frontend** (mặc định: `http://localhost:3001`).
...
1. Thủ kho (Picker) cầm điện thoại, mở **WMS Mobile Scanner** tại đường dẫn `http://localhost:3002/m/pick`.
...
3. Mở điện thoại vào **WMS Mobile Scanner** mục đóng gói: `http://localhost:3002/m/pack`.
