...
| Vị trí/Thành phần | Mô tả kịch bản lỗi | Giải pháp khắc phục cụ thể (Lean) |
| --- | --- | --- |
| **Diverged Inventory Truth (Stock trong PMI)** | ~~PMI tự lưu số lượng stock ở `ProductVariant` như một "source of truth" phụ độc lập với WMS.~~ **[RESOLVED]** Đã xóa trường `stock` khỏi PMI. Frontend Web giờ gọi WMS API (`GET /public/stock`) để lấy `qty_available` real-time. | ~~Xóa trường `stock` hoặc không sử dụng trường `stock` trong `ProductVariant` của PMI.~~ **DONE** - Migration `c9a2d4b80123_remove_stock_column.py` |
