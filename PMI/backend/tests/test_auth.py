...
def test_get_me_service_auth(client_no_auth_override, tenant_headers):
    ...
    bad_headers = {**tenant_headers, "X-API-Key": "invalid_key"}
...
def test_contextvars_isolation(
    client_no_auth_override, tenant_headers, tenant_identity_claims
):
    token1 = create_access_token({**tenant_identity_claims, "sub": "user1"})
    token2 = create_access_token({**tenant_identity_claims, "sub": "user2"})

    # Check request 1
    resp1 = client_no_auth_override.get(
        "/api/auth/me_context",
        headers={**tenant_headers, "Authorization": f"Bearer {token1}"},
    )
    ...
    # Check request 2
    resp2 = client_no_auth_override.get(
        "/api/auth/me_context",
        headers={**tenant_headers, "Authorization": f"Bearer {token2}"},
    )
    ...
    resp_service = client_no_auth_override.get(
        "/api/auth/me_context",
        headers={**tenant_headers, "X-API-Key": INTERNAL_SERVICE_TOKEN},
    )
    ...
    resp_service2 = client_no_auth_override.get(
        "/api/auth/me_context",
        headers={
            **tenant_headers,
            "X-API-Key": INTERNAL_SERVICE_TOKEN,
