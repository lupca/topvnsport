...
export default function ProductForm({ productId, duplicateProductId, onSaveSuccess }: ProductFormProps = {}) {
  ...
  // Fetch product data - chỉ lưu vào state, không reset form ở đây
  useEffect(() => {
    if (!targetId) return;

    fetchWithAuth(`/products/${targetId}`)
      .then(data => {
        setPendingProductData(data);  // Lưu tạm, đợi options load xong
      })
      .catch(err => {
        console.error("Error loading product data:", err);
        setSubmitError("Không thể tải thông tin sản phẩm.");
      });
  }, [targetId, duplicateProductId]);

  // Reset form khi CẢ options VÀ product data đã load xong
  useEffect(() => {
    ...
    // Clear pending data sau khi đã xử lý xong
    setPendingProductData(null);
  }, [optionsLoaded, pendingProductData, duplicateProductId, reset]);
