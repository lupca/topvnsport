...
export default function PromotionList() {
  ...
  const renderStatusBadge = (status: PromotionStatus) => {
    const config: Record<PromotionStatus, { bg: string; text: string; label: string; icon: any }> = {
      ...
      ACTIVE: { bg: "bg-emerald-50 text-emerald-700 border-emerald-200", label: "Đang chạy", icon: CheckCircle },
      PAUSED: { bg: "bg-amber-50 text-amber-700 border-amber-200", label: "Tạm dừng", icon: AlertCircle },
      ENDED: { bg: "bg-rose-50 text-rose-700 border-rose-200", label: "Kết thúc", icon: StopCircle },
    };
    const cfg = config[status] || { bg: "bg-gray-100 text-gray-700 border-gray-200", label: status, icon: Clock };
    const Icon = cfg.icon;

    return (
      <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-semibold border ${cfg.bg}`}>
