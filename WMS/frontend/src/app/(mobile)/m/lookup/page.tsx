    ...
    <div className="space-y-4 max-w-md mx-auto">
      ...
      <div>
        <h1 className="text-base font-bold text-gray-900 flex items-center gap-1.5">
          <Search className="w-5 h-5 text-indigo-600" /> Tra cứu (Lookup)
        </h1>
        <p className="text-[10px] text-gray-500">Quét mã vạch sản phẩm để tra cứu vị trí & số lượng tồn kho</p>
      </div>
      ...
      {/* SKU Details Display */}
      {skuInfo && !loading && (
        <div className="bg-surface border border-gray-200 shadow-sm rounded-2xl p-4 space-y-4">
          <div className="flex gap-3 items-start">
            {skuInfo.image_url ? (
              <img
                src={skuInfo.image_url}
                alt={skuInfo.product_name}
                className="w-16 h-16 rounded-xl object-cover border border-gray-200 bg-surface"
              />
            ) : (
              <div className="w-16 h-16 rounded-xl bg-surface border border-gray-200 flex items-center justify-center text-gray-500">
                <Box className="w-6 h-6" />
              </div>
            )}
            <div className="space-y-1 text-xs">
              <h3 className="font-extrabold text-gray-800">{skuInfo.product_name}</h3>
              <p className="text-gray-500 font-semibold text-[10px]">Variant: {skuInfo.variant_name}</p>
            ...
            {stocks.length === 0 ? (
              ...
              <div className="space-y-2">
                {stocks.map((item) => (
                  <div
                    key={item.id}
                    className="p-3 bg-surface border border-gray-200 rounded-xl flex items-center justify-between text-xs"
                  >
          ...
          <div className="bg-surface border border-gray-200 shadow-sm rounded-2xl w-full max-w-sm p-5 shadow-2xl space-y-4 text-gray-900 animate-in fade-in zoom-in-95 duration-150">
            ...
            <form onSubmit={handleCreateMapping} className="space-y-3.5 text-xs">
              <div className="space-y-1">
                <label className="font-bold text-gray-500 block uppercase tracking-wider text-[9px]">Mã Vạch Scanned</label>
                <input
                  type="text"
                  value={unmappedBarcode}
                  disabled
                  className="w-full p-2.5 bg-surface border border-gray-200 rounded-lg text-gray-500 font-mono disabled:opacity-50"
                />
              </div>

              <div className="space-y-1">
                <label className="font-bold text-gray-700 block uppercase tracking-wider text-[9px]">Mã SKU *</label>
                <input
                  type="text"
                  placeholder="VD: SKU-SPORTS-BLUE-M"
                  value={mappingSku}
                  onChange={(e) => setMappingSku(e.target.value)}
                  className="w-full p-2.5 bg-surface border border-gray-200 rounded-lg text-gray-900 focus:outline-none focus:border-indigo-500"
                  required
                />
              </div>

              <div className="space-y-1">
                <label className="font-bold text-gray-700 block uppercase tracking-wider text-[9px]">Tên sản phẩm *</label>
                <input
                  type="text"
              ...
              <div className="space-y-1">
                <label className="font-bold text-gray-700 block uppercase tracking-wider text-[9px]">Biến thể (Không bắt buộc)</label>
                <input
                  type="text"
              ...
              <div className="flex justify-end gap-2 border-t pt-3.5 border-gray-200">
                <button
                  type="button"
                  onClick={() => setIsMappingModalOpen(false)}
                  className="px-4 py-2 border border-gray-200 text-gray-500 font-bold rounded-xl hover:bg-gray-100"
                >
                  Hủy
                </button>
                <button
