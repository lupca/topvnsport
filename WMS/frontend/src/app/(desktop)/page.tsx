    ...
    return (
      <div className="p-8 space-y-8 animate-pulse max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[1, 2, 3].map((n) => (
            <div key={n} className="h-32 bg-slate-800 rounded-2xl"></div>
          ))}
        </div>
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-sm flex flex-col justify-between relative overflow-hidden group hover:border-slate-700 hover:shadow-md transition-all duration-200">
          <div className="absolute right-0 bottom-0 translate-x-4 translate-y-4 opacity-5 pointer-events-none group-hover:scale-110 transition-transform duration-300">
            <Home className="w-32 h-32 text-slate-700" />
          </div>
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-extrabold uppercase tracking-widest text-slate-400">
              Kho hàng hoạt động
            </span>
            <div className="w-8 h-8 rounded-lg bg-emerald-950/30 flex items-center justify-center">
              <Home className="w-4 h-4 text-emerald-400" />
            </div>
          </div>
          <div className="mt-4">
            <h3 className="text-3xl font-extrabold text-slate-100">{stats.warehouse_count}</h3>
        ...
        <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-sm flex flex-col justify-between relative overflow-hidden group hover:border-slate-700 hover:shadow-md transition-all duration-200">
          <div className="absolute right-0 bottom-0 translate-x-4 translate-y-4 opacity-5 pointer-events-none group-hover:scale-110 transition-transform duration-300">
            <MapPin className="w-32 h-32 text-slate-700" />
          </div>
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-extrabold uppercase tracking-widest text-slate-400">
              Tổng số Vị trí / Ô kệ
            </span>
            <div className="w-8 h-8 rounded-lg bg-violet-950/30 flex items-center justify-center">
              <MapPin className="w-4 h-4 text-violet-400" />
            </div>
          </div>
          <div className="mt-4">
            <h3 className="text-3xl font-extrabold text-slate-100">{stats.location_count}</h3>
            <p className="text-[10px] text-slate-400 mt-2 font-medium">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-sm flex flex-col justify-between">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h3 className="text-sm font-extrabold text-slate-100">Biểu đồ Chỉ số Kho</h3>
              <p className="text-[10px] text-slate-400 mt-0.5">So sánh định lượng tồn kho và các lệnh giao dịch</p>
            </div>
          </div>
                ...
                <BarChart data={chartData} margin={{ top: 10, right: 10, left: -25, bottom: 0 }}>
                  ...
                  <Bar dataKey="Số lượng" fill="url(#barColor)" radius={[6, 6, 0, 0]} maxBarSize={36}>
                    {chartData.map((entry, index) => (
                      <Cell
                        key={`cell-${index}`}
                        fill={entry["Số lượng"] > 0 ? "url(#barColor)" : "#334155"}
                      />
                    ))}
                  </Bar>
                </BarChart>
        ...
        <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-sm flex flex-col justify-between">
          <div className="mb-4 border-b pb-3 border-slate-800 flex items-center justify-between">
            <div>
              <h3 className="text-sm font-extrabold text-slate-100">Cảnh báo hết hàng</h3>
              <p className="text-[10px] text-slate-400 mt-0.5">Sản phẩm có tồn khả dụng ít hơn 10</p>
            </div>
          ...
          <div className="flex-1 overflow-y-auto max-h-64 space-y-2">
            {lowStockError ? (
              ...
              <div className="text-xs text-slate-400 text-center py-8">Chưa có cảnh báo nào</div>
            ) : (
              lowStockItems.map((item) => (
                <div key={item.id} className="p-3 bg-amber-950/20 border border-amber-900/50 rounded-xl flex justify-between items-center">
                  <div>
                    <p className="text-xs font-bold text-slate-200">{item.product_name}</p>
                    <p className="text-[10px] text-slate-400">SKU: {item.sku_code}</p>
                  </div>
                  <span className="text-xs font-extrabold text-amber-300 bg-amber-900/50 px-2 py-0.5 rounded">
      ...
      <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-sm">
        <div className="mb-4 flex items-center justify-between">
          <div>
            <h3 className="text-sm font-extrabold text-slate-100">10 Giao dịch kho gần nhất</h3>
            <p className="text-[10px] text-slate-400 mt-0.5">Lịch sử biến động tồn kho chi tiết</p>
          </div>
              ...
              {recentTxError ? (
                ...
                recentTx.map((tx) => (
                  <tr key={tx.id} className="border-b border-slate-800 hover:bg-slate-800/30">
                    ...
                    <td className="py-2.5 font-semibold text-slate-200">{tx.sku_code}</td>
                    <td className="py-2.5">
                      <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${
                        tx.transaction_type === "INBOUND" ? "bg-emerald-950/50 text-emerald-400" :
                        tx.transaction_type === "OUTBOUND" ? "bg-rose-950/50 text-rose-400" :
                        tx.transaction_type === "ADJUST" ? "bg-amber-950/50 text-amber-400" :
                        tx.transaction_type === "TRANSFER" ? "bg-blue-950/50 text-blue-400" :
                        "bg-slate-800 text-slate-300"
                      }`}>
                        {tx.transaction_type}
                      </span>
                    </td>
                    <td className={`py-2.5 font-bold ${tx.quantity >= 0 ? "text-emerald-400" : "text-rose-400"}`}>
                      {tx.quantity >= 0 ? `+${tx.quantity}` : tx.quantity}
                    </td>
                    <td className="py-2.5 text-slate-300">{tx.location_id}</td>
                    <td className="py-2.5 text-slate-400 italic">{tx.note || "-"}</td>
                  </tr>
                ))
              )}
            </tbody>
