...
import { popupService } from "../components/Popup/popupService";
...
describe("popupService", () => {
  ...
  test("handles prompt dialog with default value", async () => {
    const promptPromise = popupService.prompt("Nhap ten", "Default Name");
    popupService.resolveCurrent("Entered Name");

    await expect(promptPromise).resolves.toBe("Entered Name");
  });
});
