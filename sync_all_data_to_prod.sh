...
# Full local -> prod data sync for TOP VN SPORT:
# - PostgreSQL: PMI, OMS, WMS
# - MinIO bucket: pim-media
#
# Usage:
#   EC2_HOST=ec2-xx-xx-xx-xx.compute-1.amazonaws.com ./sync_all_data_to_prod.sh --yes --wipe
#
# Notes:
# - This is destructive for production data.
# - By default, image_url values pointing to localhost:19005 are normalized to EC2_HOST:19005 on prod.
...
usage() {
  cat <<'EOF'
Usage:
  EC2_HOST=<host> ./sync_all_data_to_prod.sh [options]

Options:
  --wipe                      Wipe prod schemas + MinIO bucket before importing.
  --no-normalize-image-urls   Keep localhost image URLs as-is (default: normalize to EC2 host).
...
dump_restore_db() {
  ...
  local db_name="$2"
  local remote_container="$3"

  echo "[DB] Sync $db_name ($local_container -> $remote_container)"
  docker exec "$local_container" pg_dump -U postgres -d "$db_name" --clean --if-exists --no-owner --no-privileges \
    | ssh "${SSH_OPTS[@]}" "$EC2_USER@$EC2_HOST" "sudo docker exec -i $remote_container psql -U postgres -d $db_name"
}
...
if [[ "$AUTO_YES" -ne 1 ]]; then
  echo "About to sync ALL local data to production host: $EC2_HOST"
  echo "This operation is destructive for production data."
...
if [[ "$WIPE_FIRST" -eq 1 ]]; then
  echo "[1/7] Wipe prod schemas + MinIO bucket"
  run_ssh "set -e; \
    sudo docker exec pim-db psql -U postgres -d pim_db -c \"DROP SCHEMA IF EXISTS public CASCADE; CREATE SCHEMA public; GRANT ALL ON SCHEMA public TO postgres; GRANT ALL ON SCHEMA public TO public;\"; \
    sudo docker exec oms_db psql -U postgres -d oms_db -c \"DROP SCHEMA IF EXISTS public CASCADE; CREATE SCHEMA public; GRANT ALL ON SCHEMA public TO postgres; GRANT ALL ON SCHEMA public TO public;\"; \
    sudo docker exec wms-db psql -U postgres -d wms_db -c \"DROP SCHEMA IF EXISTS public CASCADE; CREATE SCHEMA public; GRANT ALL ON SCHEMA public TO postgres; GRANT ALL ON SCHEMA public TO public;\"; \
    sudo docker exec pim-minio mc alias set local http://127.0.0.1:9000 '$MINIO_USER' '$MINIO_PASS' >/dev/null; \
    sudo docker exec pim-minio mc rm --recursive --force local/$MINIO_BUCKET || true"
else
  echo "[1/7] Skip explicit wipe (pg_dump --clean + mc mirror --remove will still converge data)"
fi

echo "[2/7] Sync PostgreSQL PMI"
dump_restore_db "pim-db" "pim_db" "pim-db"

echo "[3/7] Sync PostgreSQL OMS"
dump_restore_db "oms_db" "oms_db" "oms_db"

echo "[4/7] Sync PostgreSQL WMS"
dump_restore_db "wms-db" "wms_db" "wms-db"

if [[ "$NORMALIZE_IMAGE_URLS" -eq 1 ]]; then
  echo "[5/7] Normalize prod product_media URLs to public host"
  run_ssh "sudo docker exec pim-db psql -U postgres -d pim_db -At -c \"update product_media set image_url = replace(replace(image_url,'localhost:19005','$EC2_HOST:19005'),'127.0.0.1:19005','$EC2_HOST:19005') where image_url like '%localhost:19005%' or image_url like '%127.0.0.1:19005%';\""
else
  echo "[5/7] Skip image URL normalization"
fi

echo "[6/7] Sync MinIO bucket local->$EC2_HOST"
docker run --rm --network host --entrypoint /bin/sh minio/mc -c "
  ...
  mc mirror --overwrite --remove local/$MINIO_BUCKET prod/$MINIO_BUCKET
"
